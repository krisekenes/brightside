  ...
  return (
    <article onClick={onClick} className="bs-card" style={{ background:bg,borderRadius:12,overflow:"hidden",border:`1px solid ${C.border}`,cursor:"pointer",animationDelay:`${delay}s`,display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ padding:isMobile?"18px 18px 0":"17px 17px 0",flex:1 }}>
        ...
        <div style={{ fontFamily:"'DM Serif Display',serif",fontSize:isMobile?17:15,fontWeight:400,lineHeight:1.4,color:C.ink,marginBottom:8 }}>{story.title}</div>
      ...
      <div style={{ padding:isMobile?"12px 18px 14px":"11px 17px 13px",display:"flex",alignItems:"center",justifyContent:"space-between",borderTop:`1px solid ${C.border}`,marginTop:13,background:"rgba(128,128,128,0.03)" }}>
        ...
        <div style={{ display:"flex",gap:isMobile?6:5,alignItems:"center" }} onClick={e=>e.stopPropagation()}>
          <button onClick={onLove} style={{ display:"flex",alignItems:"center",gap:5,padding:isMobile?"9px 12px":"5px 9px",borderRadius:20,background:loved?C.amberPale:C.surface,border:`1px solid ${loved?C.amber:C.border}`,cursor:"pointer",transition:"all 0.15s",minHeight:btnSz }}>
          ...
          <button onClick={onSave}  style={{ width:btnSz,height:btnSz,borderRadius:8,background:saved?C.amberPale:C.surface,border:`1px solid ${saved?C.amber:C.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s" }}><Ic.Bookmark c={saved?C.amber:C.inkLight} f={saved}/></button>
          <button onClick={onShare} style={{ width:btnSz,height:btnSz,borderRadius:8,background:C.surface,border:`1px solid ${C.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}><Ic.Share c={C.inkLight}/></button>
          {!isMobile&&<button onClick={onUnsee} style={{ width:btnSz,height:btnSz,borderRadius:8,background:C.surface,border:`1px solid ${C.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }} title="Hide"><Ic.Eye c={C.inkLight}/></button>}
        </div>
      </div>
    </article>
  );
